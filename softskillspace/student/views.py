...
class StudentBookALessonView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            request,
            f"Lesson has been created. \
                Approval is required from {lesson.tutor}",
        )
        ...
        msg_to_tutor = (
            f"Hi {lesson.tutor}\n\n" +
            f"{lesson.student} has booked in a lesson \
                for you on {lesson_date}. " +
        ...
        msg_to_student = (
            f"Hi {lesson.student}\n\n" +
            f"Your lesson has been successfully booked with \
                {lesson.tutor} for {lesson_date} " +
            f"Please visit {url} to approve the booking.\n\n" +
            "Kind Regards\n" +
            "Soft Skill Space team")
