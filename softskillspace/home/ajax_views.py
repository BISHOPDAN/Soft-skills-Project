...
class AccceptOrDeclineCookieView(View):

    def post(self, request):
        """
        Accept or decline cookie
        """
        new_cookie_val = json.loads(request.body)
        success = False
        if new_cookie_val:
            request.session["cookie_accept"] = int(new_cookie_val["value"])
            success = True

        data = {"cookie": request.session["cookie_accept"], "success": success}
