...
from softskillspace.utils.choices import (AcademicLevel, InstitutionType,
                                          LessonPaymentStatus, LessonStatus)
...
from tutor.forms import (TutorAvailabilityForm, TutorBookALessonForm,
                         TutorForm, TutorQualificationForm,
                         TutorSubjectForm)
...
class SearchView(ListView):
    ...
    def get_queryset(self):
        query = Q(verified_at__isnull=False, verified_at__lte=timezone.now(),)
        ...
        if classification:
            query &= get_query(classification,
                               ["tutorsubject__subject__classification__name"])
        tutors = (
...
class TutorBookALessonView(
    ...
    def form_valid(self, form):
        ...
        messages.success(
            request,
            f"Lesson has been created. Approval is required from \
                {lesson.student}",
        )
...
class QualificationsDeleteView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = {"success": success, "msg": message}
...
class TutorPaymentHistoryView(
    LoginRequiredMixin,
    UserPassesTestMixin,
    TemplateView
):
    template_name = 'tutor/tutor-payment-history.html'

    def test_func(self):
        return self.request.user.is_tutor


class TutorSubjectListView(
    LoginRequiredMixin,
    UserPassesTestMixin,
    ListView,
    FormView,
    ...
    context_object_name = "tutor_subjects"
    paginate_by = 25
    form_class = TutorSubjectForm
    model = TutorSubject
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        all_subjects = Subject.items.all()
        context.update({
    ...
    def get_queryset(self):
        tutor = self.request.user.tutor
        queryset = TutorSubject.items.filter(tutor=tutor) \
    ...
    def form_valid(self, form):
        ...
        tutor_subject.tutor = request.user.tutor
        tutor_subject.levels.add(*levels)  # Unpack the list
        tutor_subject.save()

        messages.success(request, "New Subject successfully added")
        return redirect(reverse_lazy('tutor:subjects'))


class TutorSubjectDeleteView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        tutor_subject = TutorSubject.items.filter(
            id=id, tutor=self.request.user.tutor
        )
